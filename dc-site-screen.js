...
(function(w){
 ...
 w.OmegaDCScreen={mount:mount,render:render};
 if(w.GA_FIBER){var aside=document.querySelector('aside'),panel=document.createElement('section');aside.insertBefore(panel,aside.firstChild);var control=mount(panel),marker=null;
  w.GA_FIBER.map.on('click',function(e){control.select(e.latlng);if(marker)marker.remove();marker=w.GA_FIBER.L.marker(e.latlng).addTo(w.GA_FIBER.map).bindPopup('Selected site — run data-center screening');});
 }
})(window);
