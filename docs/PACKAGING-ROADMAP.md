...
| 7 · Usage and review | [#142](https://github.com/clearskyenergy/omega-core/pull/142) · [preview](https://omega-core-git-codex-packaging-phase-7-clearsky-usa.vercel.app), stacked on #141 | draft, unmerged; npm test + check:pages pass; disabled | Cycle-keyed usage counters (EV applications, site studies, BOM activity), the produce gate with the pack offer, overage lines on the recurring invoice, buy-more packs through reconciliation, auto top-up, the 90-day review on the Package tab and Your plan. Permitting matrices await their producer. See PACKAGING-PHASE-7-VALIDATION.md. |
| 8 · Omega Logic follows the package | [#143](https://github.com/clearskyenergy/omega-core/pull/143) · [preview](https://omega-core-git-codex-packaging-phase-8-clearsky-usa.vercel.app), stacked on #142 | draft, unmerged; npm test + check:pages pass; disabled | `logic-access` judges a packaged tenant by `modules[]` and the live grant; Plant, Materials & Purchasing, Logistics & Warranty and Customer App are refused by name on their endpoints; the chrome, hub, dashboard and Omega Logic app draw only the parts held; the customer portal is the Customer App part. Legacy tenants unchanged. Release checklist in PACKAGING-RELEASE-CHECKLIST.md. See PACKAGING-PHASE-8-VALIDATION.md. |
| 9 · The plant's doors and sign-in | [#144](https://github.com/clearskyenergy/omega-core/pull/144) · [preview](https://omega-core-git-codex-packaging-phase-9-clearsky-usa.vercel.app), stacked on #143 | draft, unmerged; npm test + check:pages pass; disabled | The bench, the rig and hold/release follow the Plant part for a packaged workspace; `firestore.rules` compares a sign-in email lower-cased (team blocks, terms, `isAdmin()`), which needs the one rules deploy; the preview sign-in refusal names the hostname to authorise. See PACKAGING-PHASE-9-VALIDATION.md. |
| 10B · The Ladder: pay and change the plan in settings | `codex/dashboard-billing` | draft; npm test + check:pages + check:dashboard pass; disabled | The dashboard's Account panel shows a packaged workspace its package, status, monthly membership, next invoice, amount due with Pay in QuickBooks and I've paid, and its invoices; *The Ladder* (build your own experience and pay for what you need; each Omega Logic department its own opt-in) opens the one package menu on the dashboard; a locked tile offers its module by name and price; a legacy prepaid account sees none of it. See PACKAGING-PHASE-10B-VALIDATION.md. |
| 10A · Sign up, pay in QuickBooks, activate | [#146](https://github.com/clearskyenergy/omega-core/pull/146) · [preview](https://omega-core-git-codex-self-serve-clearsky-usa.vercel.app), stacked on #145 | draft, unmerged; npm test + check:pages pass; disabled | The public price list (`GET /api/offerings`, `offerings.html`); signup pays at the end through the engine's own activation (self-serve, audited), the pay step with QuickBooks' card page and "I've paid" (`plan-change.reconcileNow`, throttled), the fallback to approval; `api/_lib/packaging-mode.js` is the ONE sandbox-or-live rule the engine, the item sync, the QuickBooks guard, the price book, signup, the runner and both scripts read; a `-proposed` book is never production. See PACKAGING-PHASE-10A-VALIDATION.md and PAYMENTS-BROWSER-SETUP.md. |
...
4. **One place for everything else: the + Modules tab, where they
   subscribe.** A single ribbon tab at the far right, **"The Ladder"** (it was "+ Modules" until 2026-09-26), shows
...
| **8 · Omega Logic follows the package** | The Logic endpoints, chrome and apps read `modules[]`: Office and its four parts gated where they are served; legacy tenants unchanged; the release checklist for the stacked merge, seeds, rules deploy and sandbox acceptance. | A packaged tenant without a part cannot reach its screens or endpoints; a tenant with it sees nothing new; every earlier phase's release steps are written down in order. |
...
- **Activate** → `POST /api/tenant-package {orgId, modules, pricebookVersion,
  ...
  **Send as proposal** → opens `subscription-proposal.html` on this tenant
  with the rail's terms (Phase 6).
...
The customer menu rendered as the tenant admin will see it. Add → the
server quote with today's prorated charge and the new monthly total, then
"Subscribe and pay" (a QuickBooks change invoice; the card waits for
payment) or "Turn it on" when the tier already covers it; "On · remove at
review" on owned modules, with removal requests queued for the quarterly
review. Built in Phase 5 on `POST /api/plan-change`; the same control as the
editor's + Modules gallery. Staff see exactly what the customer sees and any
action taken here is on the customer's behalf and billed to them.
...
| Opt-in mid-cycle | `plan-change` quotes the difference and issues one prorated **change invoice** to the next billing date (§10.5), one open change at a time; when QuickBooks shows it paid the module joins `billing/current.subscription` and the next recurring invoice bills it at full price. Built in Phase 5 (`api/_lib/plan-change.js`). |
...
1. The card shows **"$X today, then $Y/month on the <billing day>."**
   ...
   **only when that tier's current cycle is already paid in QuickBooks**
   (decided 2026-09-26, option 1). Nothing new is owed, so there is nothing
   to wait for. A trial tenant, an unpaid tenant, or a tenant whose cap the
   addition would exceed goes through the paid path below.
...
**Built (Phase 5, Step A).** Two records, one meaning each:
`billing/current.subscription` is what the tenant **bought** (modules, plan,
interval, logins, since) and is written only by approval, activation and a
paid or reversed change; `billing/current.modules` is what is **switched on**
and is derived from the subscription and the payment state of the
subscription invoices. A late payer therefore drops to Lite for a while and
never loses the package they bought. A change is its own invoice record
(`billing/current/invoices/change-<id>`, `kind: 'change'`) with its own
QuickBooks DocNumber and one of five states: `unpaid` → `paid`, `reversed`,
`expired` (the cycle rolled before it was paid) or `cancelled`. Only one
change may wait for payment at a time. A payment that arrives after the
cycle rolled or after a cancel is still honoured (the customer paid) and the
record is marked `reviewRequired` so a person invoices the gap or refunds.
Cancelling never voids the QuickBooks invoice; staff do that in QuickBooks.
A reversal of a paid change takes the modules back out unless a later paid
subscription invoice already covers them. Removals are recorded as
`removalRequests[]` for the quarterly review and change nothing today. A
trial or unpaid tenant cannot add; additions to an annual prepay are quoted
by ClearSky, not self-served. Everything is behind the same sandbox flags as
Phase 4. Step B (card on file) is not built.
...
8. A $0 addition inside a tier whose current cycle is already paid activates
   immediately; anything owed waits for the paid invoice (decided 2026-09-26,
   option 1).
