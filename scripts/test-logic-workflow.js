...
await check('tenant colors persist through the existing branding boundary and drive shared theme tokens',async function(){
  ...
  assert.deepEqual(brand(org),{name:'CleanCell Studio',shortName:'CleanCell Studio',workspace:org.name,logoUrl:'/tenants/cleancell/logo.png',primary:colors.primary,accent:colors.accent,ink:colors.ink,supportEmail:'',attribution:'Powered by ClearSky OMEGA'});assert.equal(brand(org).workspace,'Clean Cell');
  /* the customer-facing brand follows the white-label contract: off means the tenant's own name and no "powered by"; 'none' removes our line */
  assert.equal(brand({name:'Org',whiteLabel:{enabled:false,platformName:'WL'}}).name,'Org');assert.equal(brand({name:'Org',whiteLabel:{enabled:false,platformName:'WL'}}).attribution,'');
  assert.equal(brand({whiteLabel:{platformName:'WL',attribution:'none'}}).attribution,'');assert.equal(brand({whiteLabel:{platformName:'WL',attributionText:'Powered by Omega Logic'}}).attribution,'Powered by Omega Logic');
  assert.equal(brand({whiteLabel:{platformName:'WL',ink:'#0E1B24',supportEmail:'desk@example.com'}}).ink,'#0E1B24');assert.equal(brand({whiteLabel:{platformName:'WL',supportEmail:'desk@example.com'}}).supportEmail,'desk@example.com');
...
await check('account terms snapshot is applied before invoice and remains immutable',async function(){setup();db.seed('omega_orgs/cleancell.us/customer_index/buyer@example.com',{customerId:'buyer'});db.seed('omega_orgs/cleancell.us/customers/buyer',{terms:{depositPct:50,dueDays:10}});db.seed('omega_orgs/cleancell.us/customers/buyer/users/buyer@example.com',{email:'buyer@example.com',role:'owner',status:'active'});await W.price('one',1000,owner);assert.equal(db.data.get('orders/one').logic.commercial.depositCents,50125);await assert.rejects(W.price('one',2000,owner));
  /* pricing is the office's review: from here the order belongs to the customer ACCOUNT, so everyone on it sees it */
  assert.equal(db.data.get('orders/one').customerId,'buyer');});
await check('an order an anonymous visitor typed on the storefront is not tied to an account by pricing',async function(){setup();db.seed('omega_orgs/cleancell.us/customer_index/buyer@example.com',{customerId:'buyer'});db.seed('omega_orgs/cleancell.us/customers/buyer',{terms:{}});db.seed('omega_orgs/cleancell.us/customers/buyer/users/buyer@example.com',{email:'buyer@example.com',role:'owner',status:'active'});await db.doc('orders/one').update({source:'embed'});await W.price('one',1000,owner);assert.equal(db.data.get('orders/one').customerId,undefined);});
...
await check('F1: a voided serial still filed under a finished cabinet (a typo the plant corrected) does not keep it off the shelf, and is never assigned',async function(){setup();db.seed('plant_units/cleancell.us__ROOT',unit('ROOT'));db.seed('plant_units/cleancell.us__CELL',unit('CELL',{unitType:'cell',shipUnit:false,parentSerial:'ROOT',rootSerial:'ROOT',sku:'CELL'}));db.seed('plant_units/cleancell.us__CELLTYPO',unit('CELLTYPO',{unitType:'cell',shipUnit:false,parentSerial:'ROOT',rootSerial:'ROOT',sku:'CELL',inventoryStatus:'void',at:'',test:null,woId:null}));await W.price('one',1000,owner,true);payments.deposit=true;await W.processOrder('one');assert.equal(db.data.get('plant_units/cleancell.us__ROOT').orderId,'one');assert.equal(db.data.get('plant_units/cleancell.us__CELL').orderId,'one');var v=db.data.get('plant_units/cleancell.us__CELLTYPO');assert.equal(v.inventoryStatus,'void');assert.equal(v.orderId,null);assert.equal(db.data.get('plant_works_orders/wo_one').releasedUnits,2);});
...
await check('a tenant-billed order needs no QuickBooks: the office records its own invoice and the payment that landed, and release, ready and ship follow',async function(){
  ...
  var iss=await post(office,{action:'invoice-issued',orderId:'one',stage:'deposit',number:'ACME-4X7Q-0926-01 Rev B',date:'2026-09-23'},admin);assert.equal(iss.invoice.status,'awaiting_payment');
  assert.equal((await post(office,{action:'invoice-issued',orderId:'one',stage:'deposit',number:'ACME-4X7Q-0926-01 Rev B',date:'2026-09-23'},admin)).duplicate,true);
  ...
  var linked=await post(office,{action:'invoice-issued',orderId:'one',stage:'deposit',number:'ACME-4X7Q-0926-01 Rev B',date:'2026-09-23',payUrl:'https://pay.cleancell.us/inv/4X7Q'});var dep=db.data.get('orders/one').logic.invoices.deposit;
  ...
  assert.equal(dep.payUrl,'https://pay.cleancell.us/inv/4X7Q');assert.equal(dep.payUrlBy,'tom@clearsky-usa.com');assert.equal(require('../api/_lib/portal').publicOrder(db.data.get('orders/one')).checkout.invoices[0].status,'part_paid','the customer still sees part paid');
  await post(office,{action:'invoice-issued',orderId:'one',stage:'deposit',number:'ACME-4X7Q-0926-01 Rev B',date:'2026-09-23',payUrl:null});dep=db.data.get('orders/one').logic.invoices.deposit;assert.equal(dep.status,'part_paid','removing the link keeps it part paid');assert.equal(dep.payUrl,null);assert.equal(dep.issuedBy,'factory@cleancell.us');
  ...
  await post(office,{action:'invoice-issued',orderId:'one',stage:'balance',number:'ACME-4X7Q-0926-02',date:'2026-10-10'},admin);
  ...
  var pub=require('../api/_lib/portal').publicOrder(o);assert.equal(pub.checkout.invoices[0].number,'ACME-4X7Q-0926-01 Rev B');assert.equal(pub.checkout.invoices[0].status,'paid');assert.equal(pub.checkout.processingFee,0);assert.equal(pub.checkout.invoices[0].payUrl,null);
});
