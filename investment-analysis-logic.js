...
function iaWirePlatformNav(){
  var url = iaTenantPortalUrl();
  var back = document.getElementById('tb-back-link');
  var logo = document.getElementById('tb-logo-link');
  if (back) back.setAttribute('href', url);
  if (logo) logo.setAttribute('href', url);
}

/* Boot persistence after the page's own boot(). */
if (document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ iaWirePlatformNav(); iaInitPersistence(); });
else { iaWirePlatformNav(); iaInitPersistence(); }
