...
var ITEMS = [
  ...
  { key: 'office-app', audience: 'office', kind: 'app', name: 'Office app', path: '/office/app', org: true, sandbox: '/app-sandbox/office', guide: '/guides/Omega-Logic-Office-App.pdf', install: 'home-screen',
    what: 'Today, orders and customer requests, PO loads, customers, stock, and Sites: what the customer says, receive a load by scan, the unit passport. For the project manager, procurement and commissioning.', signin: 'Work Google account of the workspace' },
  ...
  { key: 'customer-app', audience: 'customer', kind: 'app', name: 'Customer app', path: '/portals/customer/app', org: true, sandbox: '/app-sandbox/customer', guide: '/guides/Omega-Logic-Customer-App.pdf', install: 'home-screen',
    what: 'Site plans in Editor Lite, orders with their milestone and warranty, purchase orders, and Sites & equipment: where each unit is going, received, assigned, commissioned. For the buyer.', signin: 'Email login link or Google; the first sign-in creates the account' },
...
var GUIDES = [
  ...
  { key: 'office', name: 'Office app guide', path: '/guides/Omega-Logic-Office-App.pdf', audience: 'office', covers: 'Install on a phone, the six tabs including Sites, what stays on the desktop.' },
...
function forOrg(org, opts) {
  ...
  return { org: org, name: opts.name || org, host: host || ORIGIN.replace('https://', ''),
    items: ITEMS.map(function (it) { return { key: it.key, audience: it.audience, kind: it.kind, name: it.name, what: it.what, signin: it.signin, install: it.install, url: url(it.path, it.org ? org : null, host), sandbox: it.sandbox ? ORIGIN + it.sandbox : null, guide: it.guide ? ORIGIN + it.guide : null }; }),
...
function message(kit, audience) {
  ...
  var lines = ['Here is your ' + kit.name + ' Omega Logic kit for ' + who + '.', ''];
  ...
  if (guide) lines.push('The attached guide (' + guide.url + ') shows how to install it and use it, and how it links to the main system.');
  lines.push('', 'Sign in: ' + (items[0] ? items[0].signin : '') + '.');
  return lines.join('\n');
}
module.exports = { ORIGIN: ORIGIN, ITEMS: ITEMS, GUIDES: GUIDES, url: url, forOrg: forOrg, message: message };
