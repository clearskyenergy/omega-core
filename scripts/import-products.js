...
function readRows(file) {
  ...
  if (lines.length < 2) die('That file has a header and no rows.');
...
var bad = read.headers.filter(function (h) { return FORBIDDEN.indexOf(h) >= 0; });
if (bad.length) {
...
function toProduct(r, i) {
  ...
  if (!(kw > 0) && !(kwh > 0)) { problems.push(sku + ': needs a kW or a kWh'); return null; }
...
var orderable = products.length;
var drawable = products.filter(function (p) { return p.widthFt && p.depthFt; }).length;
var oneline = products.filter(function (p) { return p._hasIntegrationAnswer; }).length;
var priced = products.filter(function (p) { return p.priceMode === 'list'; }).length;

console.log('\n  ' + FILE + '  →  omega_orgs/' + ORG + '/storefront/config.products');
