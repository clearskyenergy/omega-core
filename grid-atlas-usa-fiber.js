...
function draw(features,detailed){
 ...
 var b=viewport();
 var fs=features.filter(function(f){var p=f.properties;if(seen[p.id])return false;seen[p.id]=true;return intersects(f.bbox,b)&&(!stateEl.value||p.states.indexOf(stateEl.value)>=0)&&sourceMatches(p)&&(catEl.value==='all'||p.category===catEl.value);});
