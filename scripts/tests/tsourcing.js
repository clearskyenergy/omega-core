...
global.firebase = {auth:()=>({currentUser:{getIdToken:()=>Promise.resolve('test-token')}})};
...
global.fetch = (url, opts) => { const input=JSON.parse(opts.body); return Promise.resolve({ok:true, json:()=>Promise.resolve(responseFixture(input, input.supplierKey ? EST.VENDORS[input.supplierKey] : null))}); };
...
await ok('every rate in the shipped model declares a tier', async function(){
  let n = 0, bad = [];
...
await ok('a rate that names no source defaults to the WEAKEST tier', async function(){
...
/* ── the page ─────────────────────────────────────────────────────────── */
await ok('with nothing on file the panel claims 0% evidence', async function(){
  setup();
  const r = await EST.compute();
...
await ok('the estimate class says so in words, not just in a number', async function(){
  setup();
  const r = await EST.compute();
...
await ok('a firm, in-date quote counts as evidence and moves the share', async function(){
  setup();
...
await ok('a BUDGETARY number is a reference, never evidence', async function(){
  setup();
...
await ok('an EXPIRED quote stops being evidence on its expiry date', async function(){
  setup();
  EST.VENDORS.catl.basis = 'quote';
...
await ok('something remembered from a call is a planning rate', async function(){
  setup();
...
await ok('the JSON export carries a per-line audit trail', async function(){
  setup();
  const r = await EST.compute();
...
await ok('the CSV names the source on every line', async function(){
  setup();
  const r = await EST.compute();
  EST.setLast(r, EST.cpm());
  const csv = EST.toCsv();
...
await ok('every market reference carries a citation, a link and its caveat', async function(){
...
await ok('a reference with no duration split is NOT converted to $/kWh', async function(){
...
await ok('the internal band is labelled as internal, not as a benchmark', async function(){
  assert(/internal/i.test(EST.BAND_SRC || ''),
    'BAND_SRC does not say the range is internal');
  const hero = EST.marketHtml(await EST.compute());
  assert(/not because it is evidence/.test(hero),
    'the panel does not disclose that the band is not evidence');
});

await ok('the JSON export carries the references and labels the band', async function(){
  setup();
  const r = await EST.compute();
  EST.setLast(r, EST.cpm());
  const j = JSON.parse(EST.toJson());
...
await ok('both axes are reported, because duration decides the comparison', async function(){
  setup();
  const h = EST.marketHtml(await EST.compute());
