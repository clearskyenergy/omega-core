...
const context = vm.createContext({el,document:{documentElement:{dataset:{mode:'doom'}}},localStorage:{getItem:k=>saved[k],setItem:(k,v)=>saved[k]=v},OmegaDoom:{create:actor,createPortrait:actor},makeCore:()=>({set(){},say(){},resize(){}}),window:{addEventListener(){}},setMode(){}});
...
vm.runInContext("doomCharacter='classic';syncDoomCharacter()",context);assert(actors[1].dead);assert(!el('coreLayer').classList.contains('characterReady'));
vm.runInContext("doomCharacter='rigged';syncDoomCharacter()",context);actors[2].reject();await Promise.resolve();await Promise.resolve();assert(actors[2].dead);assert(el('characterStatus').textContent.includes('classic'));
console.log('PASS: character switching, stale loads, speech state, classic selection and load failure fallback');
