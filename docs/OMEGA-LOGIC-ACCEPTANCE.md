...
| Editor Lite parity | BESS/compute/EV/solar tenant switches; bounded canvas; real guide; five export views; Grid Atlas as entitled | Actual BESS placement and BOM view browser-checked. Compute/Solar target handoff, EV guide opening, and module-switch cleanup checked in the browser; all guides use the light tenant-accent presentation. Complete four-module placement/export/reload rehearsal and scoped Grid Atlas remain outstanding. Legacy financial/engineering export assumptions require review before issuance |
