...
window.CLEARSKY_CONFIG = {
  ...
  /* ── Firebase ──────────────────────────────────────────────────────────────
     ...
     use, so FENECON is a tenant inside it rather than a separate instance. The
     Firestore rules already scope by email domain via userOrg(), which resolves
     @fenecon.com to the orgId below with no rules change needed.
  ...
  tenant: {
    type:          'developer',
    orgId:         'fenecon.com',         // hard tenant lock — scopes ALL Firestore reads
    clientName:    'FENECON',
    allowedDomain: 'fenecon.com',         // primary sign-in domain

    /* FENECON runs three mail domains across its entities and the trial team is
       likely to span them:
         fenecon.com — group / US contact address (info@fenecon.com)
         fenecon.de  — FENECON GmbH, Deggendorf (sales@fenecon.de)
         fenecon.us  — FENECON Inc, South Carolina
       All three sign in and land in the SAME workspace, because orgId above is
       fixed at 'fenecon.com' regardless of which domain the address carries.
       Drop any of these if the trial should be narrower.                      */
    allowedDomains: ['fenecon.de', 'fenecon.us'],

    logo:          '/fenecon-logo.png',
    ...
    /* ── TRIAL ────────────────────────────────────────────────────────────────
       ...
       would unlock every tier-0 and tier-1 tool — far more than the four asked
