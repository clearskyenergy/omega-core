...
var root='data/usa-fiber/',manifest=null,overview=null,cache={},generation=0,sourcesById={};
var group=A.L.layerGroup().addTo(A.map);
function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}
function get(path){return fetch(root+path).then(function(r){if(!r.ok)throw new Error('Dataset unavailable ('+r.status+')');return r.json();});}
...
function draw(features,detailed){
 ...
 var b=viewport();
 var fs=features.filter(function(f){var p=f.properties;if(seen[p.id])return false;seen[p.id]=true;return intersects(f.bbox,b)&&(!stateEl.value||p.states.indexOf(stateEl.value)>=0)&&(catEl.value==='all'||p.category===catEl.value);});
 if(visible.checked)A.L.geoJSON({type:'FeatureCollection',features:fs},{renderer:A.L.canvas({padding:.2}),style:function(f){var c=f.properties.category;return {color:{existing:'#00bba9',unknown:'#7788ee',planned:'#edaa3b',inactive:'#999'}[c],weight:detailed?3:2,opacity:.9,dashArray:c==='planned'||c==='inactive'?'6,5':null};},onEachFeature:function(f,l){var p=f.properties,s=sourcesById[p.sourceId]||{};l.bindPopup('<strong>'+esc(p.name||s.name)+'</strong><br>'+esc(p.routeStatus)+'<br>'+esc(p.carrier||'Carrier not published')+'<br>'+esc(p.networkType||'Published fiber route')+'<br><a href="'+esc(p.sourceUrl||s.url)+'" target="_blank" rel="noopener">Published source</a><br>Retrieved: '+esc((p.retrievedAt||s.retrievedAt||'Unknown').slice(0,10))+'<br>Metadata updated: '+esc((p.metadataModified||s.itemModified||'Unknown').slice(0,10))+'<br>'+(detailed?'Published geometry; accuracy unverified':'Simplified overview — zoom in for detail')+(p.publishedLineParts?'<br>'+esc(p.publishedLineParts)+' published line parts in this record':'')+'<br>Carrier names as published; current ownership may differ. Building service and available capacity unconfirmed.');}}).addTo(group);
...
function refresh(){
 if(!manifest||!overview)return;var run=++generation,b=viewport();
 ...
 if(A.map.getZoom()<8){draw(overview.features,false);return;}
 ...
 Promise.all(rows.map(function(s){if(!cache[s.code])cache[s.code]=get(s.code+'.geojson').catch(function(e){delete cache[s.code];throw e;});return cache[s.code];})).then(function(all){if(run!==generation)return;var fs=[];all.forEach(function(fc){fs=fs.concat(fc.features.filter(function(f){return intersects(f.bbox,b);}));});draw(fs,true);}).catch(function(e){if(run!==generation)return;group.clearLayers();summary.textContent=e.message+'. Detailed view could not load; pan or zoom to retry.';});
}
...
catEl.onchange=refresh;visible.onchange=refresh;A.map.on('moveend',refresh);
...
Promise.all([get('manifest.json'),get('overview.geojson')]).then(function(r){manifest=r[0];overview=r[1];manifest.sources.forEach(function(s){sourcesById[s.id]=s;});var priority=['TX','GA','IL','CT','OH','FL','SC','CA'];manifest.states.slice().sort(function(a,b){var x=priority.indexOf(a.code),y=priority.indexOf(b.code);return (x<0?99:x)-(y<0?99:y)||a.name.localeCompare(b.name);}).forEach(function(s){var o=document.createElement('option');o.value=s.code;o.textContent=s.name+' · '+s.segments.toLocaleString();stateEl.appendChild(o);});var query=(window.location&&window.location.search||'').match(/[?&]state=([A-Z]{2})(?:&|$)/);if(query&&manifest.states.some(function(s){return s.code===query[1];})){stateEl.value=query[1];stateEl.onchange();}else refresh();}).catch(function(e){summary.textContent=e.message;});
})();
