/* © 2025–2026 ClearSky Energy Solutions LLC. Proprietary and Confidential.
   Server-only pricing, source attribution and estimate classification.
...
(function (root) {
  ...
  function price(input) {
    ...
    if (up.cost > 0 || (input.carryUpgrade && up.exposure)) {
      ...
      upLine = { name:"Utility-side upgrade — " + up.label, cost:upCost, lo:upCost*0.8, hi:upCost*1.35,
                 carried: up.cost === 0 };
    ...
    var upgradeView = shallow(up);
    if (up.exposure) {
      upgradeView.projectLowUsd = runBase + up.exposure[0];
      upgradeView.projectHighUsd = runBase + up.exposure[1];
      upgradeView.projectLowPerKwh = per(upgradeView.projectLowUsd);
      upgradeView.projectHighPerKwh = per(upgradeView.projectHighUsd);
    }

    return {
      kw:K, kwh:E, hours:hoursOf(input), pad:P, poiFt:FT,
      voltage:voltOf(input), voltLabel:V.label,
      divisions:divs, markups:marks, upgrade:upgradeView, upgradeLine:upLine,
  ...
  /* ── THE AACE CLASS — ONE COPY, AND THIS IS IT ────────────────────────
     A total with no class attached is the one thing this system must not
     let anybody quote. That rule was already written down THREE times: the
     bands and the thresholds in estClass() in clearsky-cost-estimator.html,
     the Class 5 case hardcoded in clearsky-sitefinder.html, and a third set
     in api/price-site.js. Three copies of a rule about how much to trust a
     number is the same coin toss this file was created to end — worse,
     actually, because the three agreed on the arithmetic and could quietly
     stop agreeing on the confidence attached to it.

     So the rule lives here, beside the loop that produces the number it
     grades. It takes plain facts rather than a page's state object, for the
     same reason price() does: a serverless function has no S.

       facts.open         how many of the drivers are still unanswered
       facts.gaps         packet gaps the caller knows about (estimator only)
       facts.posBad       the position on file is NOT the building
       facts.voltOpen     interconnection voltage unanswered
       facts.upgradeOpen  utility-side upgrade unanswered
       r                  the price() result, read ONLY for its evidence share

     Voltage and the utility upgrade get their own clauses rather than just
     counting towards `open` because either one can swamp everything else on
     the sheet; two open drivers that include one of them is not the same
     estimate as two that do not.

     The ceiling stays Class 4 on purpose. Class 3 needs a site walk, and
     nothing a form can be told reveals whether anybody stood on the ground.

     PLAIN FIRST, PRECISE SECOND. `label` is the cost-engineering name a
     lender's adviser looks for and means nothing to a building owner;
     `plain` is the same statement in words that can be acted on. The
     percentages in `plain` are derived from the band, so the two sentences
     cannot drift apart. */
  var AACE = {
    5: { n:5, lo:-0.30, hi:0.50, label:"Class 5 — concept screening" },
    4: { n:4, lo:-0.20, hi:0.30, label:"Class 4 — study estimate" }
  };
  M.AACE = AACE;

  function estimateClass(facts, r) {
    facts = facts || {};
    var open = num(facts.open) || 0;
    var gaps = num(facts.gaps) || 0;

    /* The evidence share travels INSIDE the reason. A Class 4 resting
       entirely on internal planning rates and a Class 4 where two thirds of
       the dollars come off written quotes are not the same estimate, and a
       reader deserves to be told which one they are holding. */
    var ev = (r && r.sourcing && typeof r.sourcing.evidenceShare === "number")
      ? r.sourcing.evidenceShare : 0;
    var evLine = ev > 0
      ? " " + Math.round(ev * 100) + "% of the total is priced from written "
        + "quotes or booked actuals; the rest is an internal planning rate."
      : " Nothing in this total is priced from a quote or a booked cost yet "
        + "— every rate is an internal planning figure.";

    var band, why;
    if (open >= 3 || facts.posBad === true || facts.voltOpen === true
        || facts.upgradeOpen === true) {
      band = AACE[5];
      why = facts.posBad === true
        ? "The position on file is not the building, so anything derived from "
          + "it is unreliable." + evLine
        : "Some of what decides the cost is still unanswered." + evLine;
    } else if (open >= 1 || gaps > 0) {
      band = AACE[4];
      why = "Not every driver is answered yet." + evLine;
    } else {
      band = AACE[4];
      why = "Every driver is answered." + evLine
          + " Class 3 additionally needs a site walk, which cannot be verified "
          + "from a desk.";
    }

    var plain = (band.n === 5
        ? "Early estimate, before anyone has visited."
        : "Study-grade estimate, built from answered drivers but no site walk.")
      + " The real number usually lands between "
      + Math.abs(Math.round(band.lo * 100)) + "% below and "
      + Math.round(band.hi * 100) + "% above this.";

    return { n:band.n, lo:band.lo, hi:band.hi, label:band.label,
             plain:plain, why:why, evidenceShare:ev };
  }
  M.estimateClass = estimateClass;
