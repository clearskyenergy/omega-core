...
- `/app-sandbox/customer` — the customer app (the buyer; opens on its hub)
...
review, a customer with a site plan on trial, an open invoice with the
supplier's pay link, and the account's CRM (contacts, calls and follow-ups,
documents both ways) — lives in this browser's localStorage. What you do
(answer a request, assign a unit, key in a stack of POs, issue parts at the
bench, size a system, send a PO, log a call, upload a document, issue an
invoice with a pay link, subscribe to the design tool) changes it, on this
phone only. **Reset** in the purple strip starts the sample over.
...
desktop pages the apps link to (a tap says so), pricing, e-mail, payment
(a pay link opens a placeholder page; subscribing grants the design tool at
once and comes back as Stripe would, over https; Manage says it is Stripe's
page), and documents (the kind is judged by the name, and a download is a
one-page sample PDF — no bytes are kept).
