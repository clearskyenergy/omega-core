...
var ORG = 'cleancell.us', O = 'omega_orgs/' + ORG, AMP = 'acct_acme', OTH = 'acct_other';
...
var JORDAN = buyer('jordan@acmefleet.com'), CFO = buyer('cfo@acmefleet.com'), NEWBIE = buyer('new@acmefleet.com'), OTHER = buyer('buyer@elsewhere.com');
...
function seed() {
  ...
  db.seed(O + '/customers/' + AMP, { orgId: ORG, name: 'Acme Fleet', status: 'active', source: 'office', accountType: 'company', createdAt: '2026-09-01T00:00:00Z' });
  db.seed(O + '/customers/' + AMP + '/users/jordan@acmefleet.com', { email: 'jordan@acmefleet.com', name: 'Jordan Blake', role: 'owner', status: 'active', createdAt: '2026-09-01T00:00:00Z' });
  db.seed(O + '/customers/' + AMP + '/users/cfo@acmefleet.com', { email: 'cfo@acmefleet.com', name: 'CFO', role: 'user', status: 'active', addedBy: 'jordan@acmefleet.com', createdAt: '2026-09-05T00:00:00Z' });
  db.seed(O + '/customers/' + AMP + '/users/new@acmefleet.com', { email: 'new@acmefleet.com', role: 'user', status: 'pending', requestedAt: '2026-09-20T00:00:00Z' });
  db.seed(O + '/customer_index/jordan@acmefleet.com', { customerId: AMP });
  db.seed(O + '/customer_index/cfo@acmefleet.com', { customerId: AMP });
  db.seed(O + '/customer_index/new@acmefleet.com', { customerId: AMP });
  ...
  /* Acme's orders: one stamped, one only billed to Jordan; one of Other Co's, one in another workspace */
  db.seed('orders/amp1', { orgId: ORG, orderNo: 'CC-1', customerId: AMP, status: 'accepted', createdAt: '2026-09-10T00:00:00Z', customer: { email: 'jordan@acmefleet.com' }, items: [{ sku: 'R60', name: 'R60 skid', qty: 2 }],
  ...
  db.seed('orders/amp2', { orgId: ORG, orderNo: 'CC-2', status: 'accepted', createdAt: '2026-09-12T00:00:00Z', customer: { email: 'jordan@acmefleet.com' }, items: [{ sku: 'R60', qty: 1 }],
  ...
  db.seed('orders/cross', { orgId: ORG, orderNo: 'CC-X', customerId: OTH, createdAt: '2026-09-12T00:00:00Z', customer: { email: 'jordan@acmefleet.com' }, items: [] });
...
(async function () {
  ...
  await test('a member reads and writes, a viewer only reads, another workspace and a customer are refused', async function () {
    ...
    assert.equal(r.company, 'Acme Fleet'); assert.equal(r.canEdit, true); assert.equal(r.canArchive, false);
    ...
    await rejects(call(crm, 'GET', { customerId: AMP }, JORDAN), 403, /Not your OEM workspace/);
  ...
  await test('a contact is saved, edited, made primary and archived by an administrator, never deleted', async function () {
    var a = await call(crm, 'POST', { customerId: AMP, action: 'contact-save', name: 'Bob Site', title: 'Site lead', email: 'Bob@AcmeFleet.com', phone: '555', primary: true }, PM);
    var b = await call(crm, 'POST', { customerId: AMP, action: 'contact-save', name: 'Ann Finance', email: 'ann@acmefleet.com', primary: true }, PM);
    assert.equal(a.contact.email, 'bob@acmefleet.com');
    ...
    await rejects(call(crm, 'POST', { customerId: AMP, action: 'contact-save', name: 'Dup', email: 'bob@acmefleet.com' }, PM), 409);
    ...
    var ed = await call(crm, 'POST', { customerId: AMP, action: 'contact-save', id: a.id, name: 'Bob Site', title: 'Construction manager', email: 'bob@acmefleet.com' }, PM);
  ...
  await test('a call with a follow-up is on Today until it is done; a task with no day is owed too; a note is not', async function () {
    ...
    assert.deepEqual(today.followUps.map(function (f) { return [f.company, f.subject, f.followUpAt]; }),
      [['Other Co', 'Intro', '2026-09-26'], ['Acme Fleet', 'Deposit timing', '2026-09-30'], ['Acme Fleet', 'Send the datasheet', null]], 'earliest first, an undated task last');
  ...
  await test('an order is logged against the account only when it is the account\'s', async function () {
    var ok = await call(crm, 'POST', { customerId: AMP, action: 'log', type: 'email', subject: 'Billed to Jordan, never stamped', orderId: 'amp2' }, PM);
    ...
    await rejects(call(crm, 'POST', { customerId: AMP, action: 'log', type: 'email', subject: 'x', orderId: 'cross' }, PM), 400, /not on this account/, 'billed to Jordan but stamped for another account');
  ...
  await test('the office uploads office-only, shares it, the customer sees it and downloads it; the customer\'s upload lands on the account', async function () {
    ...
    assert.equal((await call(myFiles, 'GET', {}, JORDAN)).files.length, 0, 'office-only is not the customer\'s');
    await rejects(call(myFiles, 'GET', { file: up.id }, JORDAN), 404);
    ...
    var mine = await upload(JORDAN, myFiles, 'bill.csv', b64('month,kwh\nJan,1200\n'), { category: 'utility-bill', note: 'Jan' });
    assert.equal(mine.file.from, 'customer'); assert.equal(mine.file.uploadedBy, 'jordan@acmefleet.com');
    ...
    assert.deepEqual((await call(myFiles, 'GET', {}, JORDAN)).files.map(function (f) { return f.name; }), ['bill.csv'], 'unshared: only their own');
  ...
  await test('archiving a document is an administrator\'s; it is kept and leaves both lists', async function () {
    ...
    assert.equal((await call(myFiles, 'GET', {}, JORDAN)).files.length, 0);
    await rejects(call(myFiles, 'GET', { file: up.id }, JORDAN), 404);
  ...
  await test('another account\'s customer, a pending colleague and an unverified address see nothing', async function () {
    ...
    var mine = await upload(JORDAN, myFiles, 'survey.png', PNG, { category: 'site-survey' });
    ...
    await rejects(call(myFiles, 'GET', {}, Object.assign(buyer('jordan@acmefleet.com'), { claims: { email_verified: false } })), 403, /confirm your email/);
    ...
    db.seed(O + '/customers/' + AMP + '/users/cfo@acmefleet.com', { email: 'cfo@acmefleet.com', role: 'user', status: 'disabled' });
    await rejects(call(myFiles, 'GET', {}, CFO), 403, /disabled/);
    db.seed(O + '/customers/' + AMP, { orgId: ORG, name: 'Acme Fleet', status: 'suspended' });
    await rejects(call(myFiles, 'GET', {}, JORDAN), 403, /disabled/);
  });
  ...
  await test('decided by the bytes: size, type, zip, disguised binaries, names and a spoofed path', async function () {
    ...
    await rejects(upload(JORDAN, myFiles, 'big.pdf', big.toString('base64')), 400, /2 MB/);
    await rejects(upload(PM, crm, 'setup.pdf', Buffer.from([0x4d, 0x5a, 0x90, 0, 3, 0, 0, 0]).toString('base64')), 400, /Only PDF/);
    await rejects(upload(JORDAN, myFiles, 'notes.txt', Buffer.from([0x4d, 0x5a, 0, 0, 0x50]).toString('base64')), 400, /Only PDF/);
    await rejects(upload(JORDAN, myFiles, 'bundle.zip', Buffer.concat([Buffer.from([0x50, 0x4b, 3, 4]), Buffer.from('payload.exe')]).toString('base64')), 400, /Excel/);
    await rejects(upload(JORDAN, myFiles, 'fake.xlsx', Buffer.concat([Buffer.from([0x50, 0x4b, 3, 4]), Buffer.from('[Content_Types].xml word/document.xml')]).toString('base64')), 400, /Excel/);
    ...
    var xlsx = await upload(JORDAN, myFiles, 'sites.xlsx', Buffer.concat([Buffer.from([0x50, 0x4b, 3, 4]), Buffer.from('[Content_Types].xml xl/workbook.xml')]).toString('base64'));
    assert.equal(xlsx.file.type, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    var dataUrl = await upload(JORDAN, myFiles, 'scan.pdf', 'data:application/pdf;base64,' + b64('%PDF-1.3 scan'));
  ...
  await test('a record whose path points at another account is refused rather than followed', async function () {
    ...
    await rejects(call(myFiles, 'GET', { file: 'f_bad' }, JORDAN), 404);
  ...
  await test('the same document twice is one document and spends no allowance; the 21st upload of the day is refused; the count is not on the customer record', async function () {
    var a = await upload(JORDAN, myFiles, 'po.pdf', PDF), b = await upload(CFO, myFiles, 'po.pdf', PDF);
    ...
    for (var i = 1; i < 20; i++) await upload(i % 2 ? JORDAN : CFO, myFiles, 'page' + i + '.txt', b64('page ' + i + '\n'));
    await rejects(upload(JORDAN, myFiles, 'page20.txt', b64('page 20\n')), 429, /20 documents today/);
    var usage = docsUnder(O + '/crm_upload_usage/');
    assert.equal(usage.length, 1); assert.equal(usage[0].count, 20); assert.equal(usage[0].customerId, AMP); assert.match(usage[0]._id, /^acct_acme__\d{4}-\d{2}-\d{2}$/);
    ...
    var listed = await call(myFiles, 'GET', {}, JORDAN);
  ...
  await test('a customer never sees a helper\'s message: a failed store is reworded', async function () {
    failSave = true;
    var e = await quietly(function () { return rejects(upload(JORDAN, myFiles, 'boom.pdf', PDF), 500); });
  ...
  await test('priced then accepted is two moments, together is one; custody, sites, designs and the design trial are on it; newest first', async function () {
    ...
    db.seed(O + '/sites/site_farm', { orgId: ORG, customerId: AMP, name: 'Farm A', address: { city: 'Joliet', state: 'IL' }, source: 'customer', createdAt: '2026-09-17T08:00:00Z', createdBy: 'jordan@acmefleet.com' });
    ...
    has('Jordan Blake (jordan@acmefleet.com) joined the account'); has('CFO (cfo@acmefleet.com) was added to the account');
    has('new@acmefleet.com asked to join the account');
