...
[shots, guides, src, path.join(tmp, 'office')].forEach(function (d) { fs.mkdirSync(d, { recursive: true }); });
...
var srcA = G.sourcesOf(tmp, ['/app-sandbox/office', '/omega-logic-theme.js?v=2', '/app-sandbox/sandbox.js', '/config.js', '/omega-tenant.js', '/vendor/zxing/x.js', '/logo.png', '/nowhere.js'], PAGES);
ok('sourcesOf: a sandbox page is the page it is built from; shared JS counts; the runtime, stand-ins, vendor, pictures and missing files do not', JSON.stringify(Object.keys(srcA)) === JSON.stringify(['office/app.html', 'omega-logic-theme.js']), srcA);
...
Object.keys(map).forEach(function (pdf) { used[pdf] = map[pdf].shots; });
...
var problems = G.freshness({ root: ROOT, shotsDir: path.join(GD, 'shots'), guidesDir: path.join(ROOT, 'guides'), guideDir: GD, pdfs: Object.keys(map), used: used });
