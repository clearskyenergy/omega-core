...
var srv = http.createServer(function (req, res) {
  ...
  if (post && ['/api/buyers', '/api/logic-custody', '/api/logic-office', '/api/crm'].indexOf(u) >= 0) return write(OFFICE);
...
(async function () {
  ...
  await check('office', '/omega-logic?org=cleancell.us', async function (p) {
    ...
    await p.click('#orders [data-pick="o2"]'); await p.waitForTimeout(400);
  ...
  await check('office-app', '/office/app?org=cleancell.us', async function (p) {
    ...
    ok('  a hub opens its panel under the hub: what is in it now, then where to work it', /1 open work order/.test(plantOpen) && /Work it/.test(plantOpen) && /Plant app/.test(plantOpen) && /Bench scan station/.test(plantOpen) && /CC-26-4420 · deposit/.test(moneyOpen) && /to issue/.test(moneyOpen) && /Invoices/.test(moneyOpen), [plantOpen.slice(0, 200), moneyOpen.slice(0, 200)]);
    ...
    ok('  Money groups the invoices: one to issue (tenant-billed, not on the supplier\'s paper yet), none overdue, one open with its pay link', groups.join('|') === 'To issue · 1|Overdue · 0|Open · 1' && links.indexOf('https://pay.example.com/cleancell/CC-INV-1058') >= 0, [groups, links]);
    await p.click('#view .inv [data-order="o2"]'); await p.waitForTimeout(300);
    ...
    ok('  the deposit invoice is recorded with its pay link (a javascript: link refused first) and moves from To issue to Open', /pay link must be a full https/.test(refused) && linkNow[0] === 'https://pay.example.com/cleancell/CC-INV-1070' && groups2.join('|') === 'To issue · 0|Overdue · 0|Open · 2', [refused, linkNow, groups2]);
