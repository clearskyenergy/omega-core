...
/* Rewrite-aware resolution lives in scripts/_routes.js, shared with tnav.js.
   Two checks needed it and each having its own answer is how one gets fixed
   and the other silently stays broken. */
const ROUTES = require('./_routes.js');
  ...
  while ((m = re.exec(src)) !== null) {
    ...
    if (srcAttr) {
      ...
      if (!fs.existsSync(target) && !(u.startsWith('/') && ROUTES.resolvesToFile(u))) {
